...
```text
src/lib/i18n/
├── client.ts         # クライアントサイドi18next設定（ブラウザ言語検出）
├── server.ts         # サーバーサイドi18n設定（HTTPヘッダー言語検出）
├── resources.ts      # 共通翻訳リソース定義（サーバー・クライアント共用）
├── languages.ts      # 言語設定・定数・ユーティリティ（共通）
...
- **クライアントサイド**: ブラウザ設定・Cookie連携（UX最適化）
- **完全分離**: 相互干渉を防ぐ独立したi18nextインスタンス
- **統一ストレージ**: Cookie使用でサーバー・クライアント間の完全同期

#### ⚡ パフォーマンス最適化

- **静的インポート**: 必要な翻訳リソースのみをバンドル
- **遅延初期化**: Hydrationエラー防止のためのタイミング制御
- **効率的キャッシュ**: Cookie活用でSSR・CSR間の状態同期
...
- **サーバーサイド**: Cookie → Accept-Languageヘッダー → フォールバック（en）
- **クライアントサイド**: Cookie → navigator.language → HTMLタグ → フォールバック（en）

> **💡 同期メカニズム**: Cookie を共通ストレージとして使用することで、サーバーサイドとクライアントサイドで完全に同期された言語設定を実現

## ⚙️ 各ファイルの詳細設計

### `client.ts` - クライアントサイド国際化エンジン

...
│   └── i18n/                # 国際化設定
├── utils/                   # ユーティリティ関数
│   ├── serverApi.ts         # Server Components用API通信
│   ├── file.ts              # ファイル操作
│   ├── string.ts            # 文字列操作
...
1. **Server/Client Components の分離**
   - `serverApi.ts` → Server Components専用
   - `useApiRequest.ts` → Client Components専用
...
2. **Route Groups** - `(authenticated)` フォルダで自動保護
3. **ミドルウェア** - 認証状態のリアルタイム監視
4. **Server Components** - 初期認証チェック
...
- **2025-07-13**: プロジェクト現状に合わせて全面更新
  - Next.js 15、React 19、TypeScript 5.8対応
  - Node.js 24.4.0、pnpm 10.13.1に更新
  - Route Groups認証システム追加
  - serverApi/useApiRequest詳細説明追加
  - Biome、Vitest設定更新
  - AWS ECS standalone出力対応
  - ドキュメント役割分担の明確化

プロジェクト運用ルールは随時見直し・改善します。ご意見・ご要望はIssueまたはPRでご提案ください。
